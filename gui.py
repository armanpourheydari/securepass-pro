...
class SecurePassGUI(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.tabs.addTab(self._build_checker_tab(), "🛡️ Checker")
        vault_tab = self._build_vault_tab()
    ...
    def _build_vault_tab(self):
        ...
        subtitle = QLabel("Enter your master password to access saved passwords")
        ...
        vault_title = QLabel("📂 Saved Passwords")
        vault_title.setObjectName("section_title")
        header_layout.addWidget(vault_title)
        header_layout.addStretch()
        save_new_btn = QPushButton("➕ Save New Password")
        save_new_btn.setObjectName("export_btn")
        save_new_btn.clicked.connect(self._save_new_password)
        header_layout.addWidget(save_new_btn)
        ...
        self.vault_table.cellClicked.connect(self._on_cell_clicked)
        self.vault_table.cellDoubleClicked.connect(self._on_cell_double_clicked)
        content_layout.addWidget(self.vault_table)
        btn_layout = QHBoxLayout()
        btn_layout.setSpacing(8)
        self.export_txt_btn = QPushButton("📄 Export TXT")
        self.export_txt_btn.setObjectName("export_btn")
        self.export_txt_btn.clicked.connect(self._export_vault_txt)
        btn_layout.addWidget(self.export_txt_btn)
        self.export_excel_btn = QPushButton("📊 Export Excel")
        self.export_excel_btn.setObjectName("export_btn")
        self.export_excel_btn.clicked.connect(self._export_vault_excel)
    ...
    def _on_tab_changed(self, index):
        """Handle tab change - if Vault tab is selected, show unlock screen"""
    ...
    def _unlock_vault_action(self):
        """Handle unlock button click or Enter key"""
    ...
    def _save_new_password(self):
        """Save a new password from vault tab"""
    ...
    def _on_cell_clicked(self, row, column):
        """Handle single click - copy password if column is Password"""
    ...
    def _on_cell_double_clicked(self, row, column):
        """Handle double click - edit the entry"""
    ...
    def _delete_selected_password(self):
        """Delete selected password from vault"""
